...
-- 1
SELECT nome, idade, curso FROM alunos WHERE curso = 'informatica';
-- 2
SELECT nome, disciplina FROM professores WHERE disciplina = 'administração';
-- 3
SELECT * FROM alunos ORDER BY idade DESC;
-- 4
SELECT curso, COUNT(*) AS qauntidade_alunos
FROM alunos
GROUP BY curso;
-- 5
SELECT curso, COUNT(*) AS total_cursos
FROM alunos 
GROUP BY curso
HAVING COUNT(*)> 6;
...
FROM professores 
JOIN turmas ON professores.id = turmas.id
JOIN turmas ON turmas.id_turma = turmas.id
;*/
